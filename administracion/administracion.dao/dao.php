...
class dao {
    ...
    function consultaClasificados() {
        ...
        $sql = "SELECT * FROM clasificados c INNER JOIN productos p ON c.codigoProducto = p.codigoProducto INNER JOIN imagenes i ON i.codigoProducto = p.codigoProducto INNER JOIN tallas t ON t.idtalla = p.idTalla inner join tiposproducto tp on c.idTipo = tp.idTiposProducto group by p.codigoProducto";
        $datos2 = mysql_query($sql, $cn->Conectarse());
        return $datos2;
    }
...
//                        mysql_query("COMMIT;");
    ...
    function comprobarCodigoValido2($codigo) {
        ...
        if ($contando > 0) {
            $sql = "SELECT * FROM clasificados c INNER JOIN productos p ON c.codigoProducto = p.codigoProducto INNER JOIN imagenes i ON i.codigoProducto = p.codigoProducto INNER JOIN tallas t ON t.idtalla = p.idTalla WHERE p.codigoProducto = '$codigo'";
            $datos2 = mysql_query($sql, $cn->Conectarse());
            $contando = mysql_affected_rows();
            if ($contando > 0) {
                return $datos2;
...
//    }
    ...
    function editarProducto(Producto $p, Costo $c, Tarifa $t, $idSucursal, $m3, $idMaterial, $colores) {
        include_once '../daoconexion/daoConeccion.php';
        $cn = new coneccion();
        $sqlCostos = "UPDATE costos set status = '2' WHERE codigoProducto = '" . $p->getCodigoProducto() . "' AND idSucursal= '$idSucursal' ";
        $sqlProductos = "UPDATE productos set producto = '" . $p->getProducto() . "',idMarca= '" . $p->getIdMarca() . "',idProveedor= '" . $p->getIdProveedor() . "',cantidadMaxima= '" . $p->getCantidadMaxima() . "',cantidadMinima= '" . $p->getCantidadMinima() . "',idTalla= '" . $p->getIdGrupoProducto() . "',idUnidadMedida= '" . $p->getIdUnidadMedida() . "',idStatus='1', idMaterial ='" . $idMaterial . "' WHERE codigoProducto = '" . $p->getCodigoProducto() . "'";
        $fecha = date("d/m/Y");
        ...
        $sqlEliminarColores = "DELETE FROM Colores WHERE codigoProducto = '" . $p->getCodigoProducto() . "'";
        mysql_query("START TRANSACTION;");
        ...
        $datos = mysql_query($sqlEliminarColores, $cn->Conectarse());
        if ($datos == false) {
            mysql_query("ROLLBACK;", $cn->Conectarse());
        } else {
            for ($x = 0; $x < count($colores); $x++) {
                $sqlInsertarColores = "INSERT INTO Colores (color, codigoProducto) VALUES ('" . $colores[$x] . "', '" . $p->getCodigoProducto() . "')";
                mysql_query($sqlInsertarColores, $cn->Conectarse());
            }
...
//                        $resultado = mysql_query($sql, $cn->Conectarse());
    ...
    function consultandoProductoPorCodigo($codigo, $sucursal) {
        include_once '../daoconexion/daoConeccion.php';
        $cn = new coneccion();
        $sql = "SELECT p.codigoProducto, p.codigoBarrasProducto, p.producto, m.marca, pr.nombre, c.costo, t.talla, p.codigoBarrasProducto, p.idProducto,c.fechaMovimiento, e.cantidad, p.cantidadMinima, p.cantidadMaxima, p.idMarca, p.idProveedor, p.idTalla, t.talla,p.idUnidadMedida, p.metrosCubicos, p.idMaterial 
                FROM productos p
        ...
        if ($row < 1) {
            $sql2 = "SELECT p.codigoProducto, p.codigoBarrasProducto, p.producto, m.marca, pr.nombre, c.costo, p.codigoProducto, p.codigoBarrasProducto, p.idProducto,c.fechaMovimiento, e.cantidad, p.cantidadMinima, p.cantidadMaxima, p.idMarca, p.idProveedor, t.idtalla, g.grupoProducto,p.idUnidadMedida, p.metrosCubicos \n"
                    . "FROM productos p\n"
                    . "INNER JOIN marcas m ON p.idMarca = m.idMarca\n"
                    . "INNER JOIN proveedores pr ON pr.idProveedor = p.idProveedor\n"
                    . "INNER JOIN costos c ON c.codigoProducto = p.codigoProducto\n"
                    . "INNER JOIN existencias e ON e.codigoProducto = p.codigoProducto\n"
                    . "INNER JOIN tallas g ON t.idTalla = p.idTalla"
    ...
    function consultandoProductoPorCodigoBarras($codigo, $sucursal) {
        include_once '../daoconexion/daoConeccion.php';
        $cn = new coneccion();
        $sql = "SELECT p.producto, m.marca, pr.nombre, c.costo, p.codigoProducto, p.codigoBarrasProducto, p.idProducto,c.fechaMovimiento, e.cantidad, p.cantidadMinima, p.cantidadMaxima, p.idMarca, p.idProveedor, t.idtalla, g.grupoProducto,p.idUnidadMedida, e.cantidad, p.metrosCubicos \n"
                . "FROM productos p\n"
                . "INNER JOIN marcas m ON p.idMarca = m.idMarca\n"
                . "INNER JOIN proveedores pr ON pr.idProveedor = p.idProveedor\n"
                . "INNER JOIN costos c ON c.codigoProducto = p.codigoProducto\n"
                . "INNER JOIN existencias e ON e.codigoProducto = p.codigoProducto\n"
                . " INNER JOIN tallas t ON t.idtalla = p.idTalla where c.status=1 and p.codigoBarrasProducto = '$codigo'  and c.idSucursal = '$sucursal' and e.idSucursal = '$sucursal'";
        $datos = mysql_query($sql, $cn->Conectarse());
        if ($datos == false) {
            $datos = mysql_error();
            $rs = 0;
...
//    . "Right Join listaPrecios l ON l.idListaPrecio = t.idListaPrecio LIMIT 0, 30 ";
    ...
    function guardarProducto(Producto $p, Costo $c, Tarifa $t, $idSucursal, $m3, $colores, $idGrupoProducto) {
        include_once '../daoconexion/daoConeccion.php';
        $cn = new coneccion();
        $cont = 0;
        ...
        for ($x = 0; $x < count($colores); $x++) {
            $slqInsertarColores = "INSERT INTO Colores (color, codigoProducto) VALUES ('$colores[$x]','" . $p->getCodigoProducto() . "')";
            mysql_query($slqInsertarColores, $cn->Conectarse());
        }
        ...
        if ($resultado == false) {
            ...
            $sql = "INSERT INTO productos(producto, idMarca, idProveedor, codigoBarrasProducto, codigoProducto,cantidadMaxima, cantidadMinima,idTalla, idUnidadMedida,idStatus, metrosCubicos, idMaterial, idGrupoProducto)VALUES('" . addslashes($p->getProducto()) . "','" . $p->getIdMarca() . "','" . $p->getIdProveedor() . "', '" . $p->getCbarras() . "' , '" . $p->getCodigoProducto() . "', '" . $p->getCantidadMaxima() . "', '" . $p->getCantidadMinima() . "', '" . $p->getIdGrupoProducto() . "', '" . $p->getIdUnidadMedida() . "','1', '$m3', '" . $p->getMaterial() . "', '" . $idGrupoProducto . "')";
            $resultado = mysql_query($sql, $cn->Conectarse());
            if ($resultado == false) {
                mysql_query("ROLLBACK;");
            } else {
...
//                        $resultado = mysql_query($sql, $cn->Conectarse());
    ...
    function consultaProducto($idSucursal) {
        ...
        $sql = "SELECT p.producto, m.marca, pr.nombre, c.costo, p.codigoProducto, p.codigoBarrasProducto, p.idProducto,c.fechaMovimiento, e.cantidad, p.cantidadMinima, p.cantidadMaxima, p.idMarca, p.idProveedor, p.idTalla, t.talla, e.idSucursal 
        FROM productos p
        INNER JOIN marcas m ON p.idMarca = m.idMarca
        INNER JOIN proveedores pr ON pr.idProveedor = p.idProveedor
        INNER JOIN costos c ON c.codigoProducto = p.codigoProducto
        INNER JOIN existencias e ON e.codigoProducto = p.codigoProducto
        INNER JOIN tallas t ON t.idtalla = p.idTalla where status=1 and p.idStatus = '1' AND c.idSucursal = '$idSucursal' AND e.idSucursal = '$idSucursal'";
...
//===================Para guardar XMl entrada===================================
    ...
    function superMegaGuardadorOrdenes($lafecha, Encabezado $encabezado, $arrayDetalleEntrada, Comprobante $comprobante, $conceptos, $control, $idSucursal, $tipo, $nombrecliente) {
        $detalle = new Detalle();
        //======================================================================
...
//        Modificar esta parte y adapatarlo correctamente
            ...
            if ($tipo !== "PEDIDO CLIENTE" && $tipo !== "ORDEN COMPRA") {
                $sqlComprobanteGuardar = "INSERT INTO xmlcomprobantes (fechaComprobante, subtotalComprobante, sdaComprobante, rfcComprobante, desctFacturaComprobante, desctProntoPagoComprobante, desctGeneralComprobante, desctPorProductosComprobante, desctTotalComprobante, ivaComprobante, totalComprobante, folioComprobante, tipoComprobante, fechaMovimiento, idSucursal, idTipoPago, nombreCliente)"
                        . " VALUES ('" . $encabezado->getFecha() . "','" . $encabezado->getSubtotal() . "','" . $comprobante->getSda() . "','" . $encabezado->getRfc() . "','" . $comprobante->getDescuentoFactura() . "','" . $comprobante->getDescuentoProntoPago() . "','" . $comprobante->getDescuentoGeneral() . "','" . $comprobante->getDescuentoPorProducto() . "','" . $comprobante->getDescuentoTotal() . "','" . $comprobante->getConIva() . "','" . $comprobante->getTotal() . "','" . $comprobante->getFolio() . "','$tipo','$lafecha','$idSucursal', '1', '$nombrecliente')";
                ...
                $sqlComprobanteGuardar = "INSERT INTO xmlcomprobantes (fechaComprobante, subtotalComprobante, sdaComprobante, rfcComprobante, desctFacturaComprobante, desctProntoPagoComprobante, desctGeneralComprobante, desctPorProductosComprobante, desctTotalComprobante, ivaComprobante, totalComprobante, folioComprobante, tipoComprobante, fechaMovimiento, idSucursal, statusOrden, idTipoPago, nombreCliente)"
                        . " VALUES ('" . $encabezado->getFecha() . "','" . $encabezado->getSubtotal() . "','" . $comprobante->getSda() . "','" . $encabezado->getRfc() . "','" . $comprobante->getDescuentoFactura() . "','" . $comprobante->getDescuentoProntoPago() . "','" . $comprobante->getDescuentoGeneral() . "','" . $comprobante->getDescuentoPorProducto() . "','" . $comprobante->getDescuentoTotal() . "','" . $comprobante->getConIva() . "','" . $comprobante->getTotal() . "','" . $sqlfolios . "','$tipo','$lafecha','$idSucursal','$estatus', '1', '$nombrecliente')";
                $sqlComprobanteId = "SELECT LAST_INSERT_ID() ID;";
                $ctrlComprobanteGuardar = mysql_query($sqlComprobanteGuardar);
                if ($ctrlComprobanteGuardar == false) {
                    mysql_query("ROLLBACK;");
...
//        }
    ...
    function consultaBuscador($idsucursal) {
        include_once '../daoconexion/daoConeccion.php';
        $cn = new coneccion();
        $sql = "SELECT p.codigoProducto, p.producto, m.marca, pr.nombre  AS proveedor, t.talla, ex.cantidad AS existencia, tf.tarifa AS menudeo\n"
                . "FROM productos p\n"
                . "INNER JOIN marcas m ON p.idMarca = m.idMarca\n"
                . "INNER JOIN proveedores pr ON pr.idProveedor = p.idProveedor\n"
                ...
                . "INNER JOIN tallas t ON t.idTalla = p.idTalla WHERE p.idStatus = '1' AND ex.idSucursal = '$idsucursal' AND tf.idListaPrecio = '1' AND tf.idSucursal='$idsucursal' AND tf.idStatus = '1'";
        $datos = mysql_query($sql, $cn->Conectarse());
//        $validando = mysql_affected_rows();
//        if ($validando >= 0) {
//            return $datos;
...
//        }
    ...
    function consultaBuscadorPorProveedor($idsucursal, $proveedor) {
        ...
        $sql = "SELECT p.codigoProducto, p.producto, m.marca, pr.nombre   AS proveedor, t.talla, ex.cantidad AS existencia, tf.tarifa AS menudeo\n"
                ...
                . "INNER JOIN existencias ex ON ex.codigoProducto =  p.codigoProducto\n"
                . "INNER JOIN tarifas tf ON tf.codigoProducto = p.codigoProducto\n"
                . "INNER JOIN tallas t ON t.idtalla = p.idTalla WHERE p.idStatus = '1' AND ex.idSucursal = '$idsucursal' AND tf.idListaPrecio = '2' AND tf.idSucursal='$idsucursal' AND tf.idStatus = '1' and pr.rfc='$proveedor'";
        $datos = mysql_query($sql, $cn->Conectarse());
        $validando = mysql_affected_rows();
        if ($validando >= 0) {
            return $datos;
...
//    Boutioque de chucho
    ...
    function dameMaterial() {
        include_once '../daoconexion/daoConeccion.php';
        $cn = new coneccion();
        $sql = "SELECT * FROM tiposMaterial";
        $rs = mysql_query($sql, $cn->Conectarse());
        return $rs;
    }

    function dameColores($codigo) {
        $sql = "SELECT * FROM Colores WHERE  codigoProducto='$codigo'";
        $rs = mysql_query($sql);
        return $rs;
    }
