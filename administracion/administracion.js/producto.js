//============ UTILIDADES ======================================================
var codigosDeColores = new Array();
...
function eliminarTablaColores() {
    for (var x = 0; x < codigosDeColores.length; x++) {
        $("tr[id='" + codigosDeColores[x] + "']").remove();
    }
    codigosDeColores.length = 0;
}
...
$("#btnAgregarColor").click(function () {
    ...
    var nuevoColor = color.substring(1, color.length);
    var dato = "<tr id='" + nuevoColor + "'>\n\
                    <td style='background-color:" + color + "'></td>\n\
                    <td><a onclick='eliminarProductoRegistro(\"" + nuevoColor + "\")' >\n\
                    <span class='glyphicon glyphicon-trash'></span></a>\n\
                    </td>\n\
                </tr>";
    var ok = validarColores(nuevoColor);
    if (ok == false) {
        $("#tablaColores").append(dato);
        codigosDeColores.push(nuevoColor);
    }
    else {
        alertify.error("El color de la prenda ya esta disponible");
    }
});

function eliminarProductoRegistro(dato) {
    for (var x = 0; x < codigosDeColores.length; x++) {
        if (dato == codigosDeColores[x]) {
            $("tr[id='" + dato + "']").remove();
            codigosDeColores.splice(x, 1);
            break;
        }
    }
}
...
function  validarColores(color) {
    var ok = false;
    for (var x = 0; x < codigosDeColores.length; x++) {
        if (color == codigosDeColores[x]) {
            ok = true;
            break;
        }
    }
            ...
            $.get('verificandoProducto.php', info, function (x) {
                if (x < 1) {
                    ...
                    $.each(lista, function (ind, elem) {
                        ...
                        if (ind == "idMaterial") {
                            $("#selectMaterial").selectpicker('val', elem);
                        }
                        if (ind == "costo") {
                    ...
                    var informacion = "codigo=" + codigo;
                    $.get("obtenerColores.php", informacion, function (datos) {
                        var datosJson = eval(datos);
                        for (var i in datosJson) {
                            codigosDeColores.push(datosJson[i]);
                            var dato = "<tr id='" + datosJson[i] + "'><td style='background-color:#" + datosJson[i] + "'></td><td><td><a onclick='eliminarProductoRegistro(\"" + datosJson[i] + "\")' ><span class='glyphicon glyphicon-trash'></span></a>\n\
                            </td></td></tr>";
                            $("#tablaColores").append(dato);
                        }
                    });
...
$("#guardarDatos").click(function () {
    ...
    if (tipoProducto !== "0" && cbarras !== "" && nombreProducto !== "" && marca !== "" && proveedor !== "" && codigoProducto !== "" && costoProducto !== "" && lista !== "" && min !== "" && max !== "" && lista !== " " && lista !== null && lista !== undefined && unidadMedida !== "" && grupoProducto !== 0 && marca !== "0" && proveedor !== "0" && grupoProducto !== "0" && unidadMedida !== "0" && unidadMedida !== "0" && material !== 0) {
        if (codigoProducto.length == 0) {
            alertify.error("Se requiere los colores disponibles");
        }
        else {
            if (min < max) {
                var listaColores = JSON.stringify(codigosDeColores);

                var info = "producto=" + nombreProducto + "&cbarras=" + cbarras + "&marca=" + marca + "&proveedor=" + proveedor + "&codigoProducto=" + codigoProducto + "&costoProducto=" + costoProducto + "&lista=" + lista + "&min=" + min + "&max=" + max + "&grupoProducto=" + grupoProducto + "&unidadMedida=" + unidadMedida + "&granel=" + granel + "&contenido=" + contenido + "&original=" + original + "&material=" + material + "&colores=" + listaColores + "&tipoProducto=" + tipoProducto;
                $.get('guardarProducto.php', info, function (x) {
                    if (x == 1) {
                        ...
                        $('#selectGrupo').selectpicker('val', 0);
                        $('#selectMedida').selectpicker('val', 0);
                        $("#selectMaterial").selectpicker('val', 0);
                        $("#txtCostoProducto").val("");
                        $("#txtCantidadMinima").val("");
                        $("#txtCantidadMaxima").val("");
...
$('#editarDatos').click(function () {
    ...
    if (nombreProducto !== "" && marca !== "" && proveedor !== "" && codigoProducto !== "" && costoProducto !== "" && lista !== "" && min !== "" && max !== "" && lista !== " " && lista !== null && lista !== undefined && unidadMedida !== "" && grupoProducto !== "" && marca !== "0" && proveedor !== "0" && grupoProducto !== "0" && unidadMedida !== "0" && unidadMedida !== "0" && idMaterial !== "0" && codigosDeColores.length !== 0) {
        if (min < max) {
            ...
            var colores = JSON.stringify(codigosDeColores);
            var info = "producto=" + nombreProducto + "&marca=" + marca + "&proveedor=" + proveedor + "&codigoProducto=" + codigoProducto + "&costoProducto=" + costoProducto + "&lista=" + lista + "&min=" + min + "&max=" + max + "&grupoProducto=" + grupoProducto + "&unidadMedida=" + unidadMedida + "&idMaterial=" + idMaterial + "&colores=" + colores;
            $.get('editarProducto.php', info, function (x) {
                ...
                callbacks.add(eliminarTablaColores());
//                callbacks.add(codigosDeColores.length = 0);
                return false;
            });
        } else {
            alertify.error("La cantidad maxima debe ser mayor a la minima");
